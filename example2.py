...
#sugita
from models import dbsession #models.py

define("port", default=8888, help="run on the given port", type=int)
define("facebook_app_id", default="App ID/API Key", help="Facebook Application ID")
define("facebook_app_secret", default="App Secret", help="Facebook Application Secret")
...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def get_current_user(self):
        ...
        #sugita
        user = engine.execute(("SELECT * FROM users id = %s", cookie["uid"]))
        ...
        if not user:
            ...
            self.db.execute("REPLACE INTO users (id, name, profile_url, access_token) "
                            "VALUES (%s,%s,%s,%s)", profile["id"], profile["name"],
                            profile["link"], cookie["access_token"])
            user = self.db.execute("SELECT * FROM users WHERE id = %s", profile["id"])
